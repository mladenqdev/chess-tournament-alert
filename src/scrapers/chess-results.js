...
async function scrapeFederation(fed) {
  ...
  $('a[href*="tnr"]').each((_, el) => {
    ...
    if (/dec[aeiou]|de[cč]j|devojc|devojk|mladih|mlad[iđ]|kadet|junior|dece|omladin|razred|srednjošk|srednjosk|osnov[ncaei]|škol|skol|liga\b|ekipn|kategorn|biznis|prvenstvo.*škol|prvenstvo.*skol/i.test(latinName)) return;
