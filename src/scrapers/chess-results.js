...
async function scrapeFederation(fed) {
  ...
  $('a[href*="tnr"]').each((_, el) => {
    ...
    if (/dec[aeiou]|de[cč]j|devojc|devojk|mladih|kadet|junior|dece|srednjošk|srednjosk|osnov[ncaei]|škol|skol|liga\b|ekipn|kategorn|prvenstvo.*škol|prvenstvo.*skol/i.test(latinName)) return;
